...
// Timings are normalised 0..1; `duration` is the length of one cycle at 1× speed.
//
// Anything stated here that is NOT in data/content_*.py is marked with `needsReview`
// and shown with a "يُراجع" badge in the UI (PROMPT.md §9).

import { LANDMARKS as L } from './geometry';
...
const BASE: Omit<Keyframe, 't' | 'phase'> = { tongue: 'rest', lips: 'rest', jaw: 0.25, velum: 'closed' };
...
function plosive(o: Opts): Keyframe[] {
  const held: Partial_ = { tongue: o.shape, contact: o.contact, contactKind: 'touch', lips: o.lips ?? 'rest', jaw: o.jaw ?? 0.2, heavy: o.heavy, top: o.top };
  const rel: Partial_ = { tongue: o.releaseShape ?? 'rest', lips: o.lips === 'closed' ? 'open' : (o.lips ?? 'rest'), jaw: (o.jaw ?? 0.2) + 0.25, heavy: o.heavy, top: o.top, airflow: o.airflow ?? 'oral' };
  return [
    kf(0, 'rest', REST),
    kf(0.18, 'approach', { jaw: o.jaw ?? 0.2, lips: o.lips ?? 'rest', top: o.top }),
...
function fricative(o: Opts): Keyframe[] {
  const held: Partial_ = { tongue: o.shape, contact: o.contact, contactKind: o.contactKind ?? 'near', lips: o.lips ?? 'rest', jaw: o.jaw ?? 0.25, heavy: o.heavy, top: o.top, airflow: o.airflow ?? 'channel' };
  return [
    kf(0, 'rest', REST),
    kf(0.2, 'approach', { jaw: o.jaw ?? 0.25, lips: o.lips ?? 'rest', top: o.top }),
    kf(0.34, 'hold', held),
    kf(0.72, 'hold', held),
    kf(0.86, 'release', { jaw: o.jaw ?? 0.25, lips: o.lips ?? 'rest' }),
    kf(1, 'rest', REST),
  ];
}
...
function nasal(o: Opts): Keyframe[] {
  const held: Partial_ = { tongue: o.shape, contact: o.contact, contactKind: 'touch', lips: o.lips ?? 'rest', jaw: o.jaw ?? 0.2, velum: 'open', airflow: 'nasal', top: o.top };
  return [
    kf(0, 'rest', REST),
    kf(0.18, 'approach', { jaw: o.jaw ?? 0.2, lips: o.lips ?? 'rest', velum: 'open', top: o.top }),
    kf(0.32, 'hold', held),
    kf(0.74, 'hold', held),
    kf(0.86, 'release', { jaw: (o.jaw ?? 0.2) + 0.2, lips: o.lips === 'closed' ? 'open' : (o.lips ?? 'rest') }),
    kf(1, 'rest', REST),
  ];
}
...
function vowel(o: Opts): Keyframe[] {
  const held: Partial_ = { tongue: o.shape, lips: o.lips ?? 'open', jaw: o.jaw ?? 0.8, heavy: o.heavy, contact: o.contact, contactKind: o.contactKind ?? 'near', airflow: o.airflow ?? 'oral', top: o.top };
  return [
    kf(0, 'rest', REST),
    kf(0.18, 'approach', { jaw: (o.jaw ?? 0.8) * 0.6, lips: o.lips ?? 'open' }),
    kf(0.3, 'hold', held),
    kf(0.78, 'hold', held),
    kf(0.9, 'release', { jaw: 0.3, lips: 'rest' }),
    kf(1, 'rest', REST),
  ];
}
...
function flap(heavy: boolean): Keyframe[] {
  const tap: Partial_ = { tongue: 'tip_flap', contact: L.ridge, contactKind: 'touch', jaw: 0.3, heavy, top: { highlight: { tip: 1 }, ...(heavy ? { highlight: { tip: 1, back: 0.5 } } : {}) } };
  const off: Partial_ = { tongue: heavy ? 'heavy' : 'rest', jaw: 0.4, heavy, airflow: 'oral', top: { highlight: { tip: 0.4 } } };
  return [
    kf(0, 'rest', REST),
    kf(0.22, 'approach', { jaw: 0.3, heavy }),
    kf(0.36, 'contact', tap),
    kf(0.42, 'contact', tap),
    kf(0.5, 'release', off),
...
function trillWrong(): Keyframe[] {
  const tap: Partial_ = { tongue: 'tip_flap', contact: L.ridge, contactKind: 'touch', jaw: 0.3, top: { highlight: { tip: 1 } } };
  const off: Partial_ = { tongue: 'tip_ridge_off', jaw: 0.3, airflow: 'oral', top: { highlight: { tip: 0.6 } } };
  const frames: Keyframe[] = [kf(0, 'rest', REST), kf(0.2, 'approach', { jaw: 0.3 })];
  let t = 0.28;
  for (let i = 0; i < 4; i++) {
    frames.push(kf(t, 'contact', tap), kf(t + 0.05, 'contact', tap), kf(t + 0.09, 'release', off));
    t += 0.13;
  }
  frames.push(kf(0.88, 'release', { jaw: 0.3 }), kf(1, 'rest', REST));
  return frames;
}

const TOP_TIP = { highlight: { tip: 1 } };
const TOP_BACK = { highlight: { back: 1 }, width: 0.3 };
...
export const ARTICULATIONS: Articulation[] = [
  ...
  art({ id: 'alif_madd', letters: 'ا', nameAr: 'الألف (مد)', group: 'jawf', duration: 3200, harakat: 2, tickerLabel: 'مد',
    keyframes: vowel({ shape: 'alif_open', lips: 'open', jaw: 0.9, airflow: 'jawf' }),
  ...
  art({ id: 'waw_madd', letters: 'و', nameAr: 'الواو (مد)', group: 'jawf', duration: 3200, harakat: 2, tickerLabel: 'مد',
    keyframes: vowel({ shape: 'waw', lips: 'rounded', jaw: 0.35, airflow: 'jawf' }),
  ...
  art({ id: 'ya_madd', letters: 'ي', nameAr: 'الياء (مد)', group: 'jawf', duration: 3200, harakat: 2, tickerLabel: 'مد',
    keyframes: vowel({ shape: 'kasrah', lips: 'spread', jaw: 0.35, airflow: 'jawf', top: { highlight: { middle: 0.5 } } }),
    ruleId: 's3r10', refImages: ['jawf'] }),

  // ---- الحلق -----------------------------------------------------------------
  art({ id: 'hamza', letters: 'ء', nameAr: 'الهمزة', group: 'throat',
    keyframes: plosive({ shape: 'throat', contact: L.throat1, jaw: 0.3, airflow: 'throat', releaseShape: 'throat' }),
  ...
  art({ id: 'ha', letters: 'هـ', nameAr: 'الهاء', group: 'throat',
    keyframes: fricative({ shape: 'throat', contact: L.throat1, contactKind: 'near', jaw: 0.35, airflow: 'throat' }),
    ruleId: 's3r1', refImages: ['throat_zones'] }),
  art({ id: 'ain', letters: 'ع', nameAr: 'العين', group: 'throat',
    keyframes: fricative({ shape: 'throat', contact: L.throat2, contactKind: 'near', jaw: 0.35, airflow: 'throat' }),
    ruleId: 's3r1', refImages: ['throat_zones'] }),
  art({ id: 'hha', letters: 'ح', nameAr: 'الحاء', group: 'throat',
    keyframes: fricative({ shape: 'throat', contact: L.throat2, contactKind: 'near', jaw: 0.35, airflow: 'throat' }),
    ruleId: 's3r1', refImages: ['throat_zones'] }),
  art({ id: 'ghain', letters: 'غ', nameAr: 'الغين', group: 'throat',
    keyframes: fricative({ shape: 'back_near', contact: L.uvula, contactKind: 'near', jaw: 0.3, heavy: true, airflow: 'throat', top: TOP_HEAVY }),
  ...
  art({ id: 'qaf', letters: 'ق', nameAr: 'القاف', group: 'back',
    keyframes: plosive({ shape: 'back_soft', contact: L.softPalate, heavy: true, top: TOP_BACK }),
    ruleId: 's3r2', refImages: ['qaf', 'top_back'] }),
  art({ id: 'kaf', letters: 'ك', nameAr: 'الكاف', group: 'back',
    keyframes: plosive({ shape: 'back_hard', contact: L.hardSoftBoundary, top: TOP_BACK }),
    ruleId: 's3r2', refImages: ['kaf', 'top_back'] }),

  // ---- وسط اللسان -------------------------------------------------------------
  art({ id: 'jim', letters: 'ج', nameAr: 'الجيم', group: 'middle',
    keyframes: plosive({ shape: 'mid_palate', contact: L.hardPalateMid, top: TOP_MID }),
    ruleId: 's3r3', refImages: ['jim_shin_ya'] }),
  art({ id: 'shin', letters: 'ش', nameAr: 'الشين', group: 'middle',
    keyframes: fricative({ shape: 'mid_channel', contact: L.hardPalateMid, contactKind: 'near', airflow: 'mid_channel', top: TOP_MID }),
    ruleId: 's3r3', refImages: ['jim_shin_ya'] }),
  art({ id: 'ya', letters: 'ي', nameAr: 'الياء', group: 'middle',
    keyframes: fricative({ shape: 'mid_ya', contact: L.hardPalateMid, contactKind: 'near', airflow: 'mid_channel', lips: 'spread', top: { highlight: { middle: 0.7 } } }),
    ruleId: 's3r3', refImages: ['jim_shin_ya'] }),

  // ---- حافة اللسان ------------------------------------------------------------
  art({ id: 'dad', letters: 'ض', nameAr: 'الضاد', group: 'side', duration: 3200,
    keyframes: fricative({ shape: 'dad', heavy: true, jaw: 0.2, airflow: 'lateral', top: { width: 1, highlight: { sides: 1, back: 0.4 } } }),
    ruleId: 's3r4', refImages: ['top_sides', 'heavy'] }),

  // ---- طرف اللسان مع اللثة ------------------------------------------------------
  art({ id: 'lam', letters: 'ل', nameAr: 'اللام', group: 'tip_gum',
    keyframes: fricative({ shape: 'tip_ridge', contact: L.ridge, contactKind: 'touch', airflow: 'lateral', top: { highlight: { front_edge: 1 }, lateral: 1 } }),
    ruleId: 's3r5', refImages: ['lam_nun_ra', 'top_front_edge'] }),
  art({ id: 'nun', letters: 'ن', nameAr: 'النون', group: 'tip_gum',
    keyframes: nasal({ shape: 'tip_ridge', contact: L.ridge, top: TOP_TIP }),
  ...
  art({ id: 'ta', letters: 'ت', nameAr: 'التاء', group: 'tip_root',
    keyframes: plosive({ shape: 'tip_teeth_root', contact: L.teethRoot, top: TOP_TIP }),
  ...
  art({ id: 'tta', letters: 'ط', nameAr: 'الطاء', group: 'tip_root',
    keyframes: plosive({ shape: 'tip_teeth_root_heavy', contact: L.teethRoot, heavy: true, releaseShape: 'heavy', top: { highlight: { tip: 1, back: 0.5 } } }),
  ...
  art({ id: 'sin', letters: 'س', nameAr: 'السين', group: 'tip_lower', duration: 3000,
    keyframes: fricative({ shape: 'tip_lower', contact: L.lowerTeeth, airflow: 'channel', lips: 'spread', top: { highlight: { tip: 0.7 } } }),
  ...
  art({ id: 'sad', letters: 'ص', nameAr: 'الصاد', group: 'tip_lower', duration: 3000,
    keyframes: fricative({ shape: 'tip_lower_heavy', contact: L.lowerTeeth, airflow: 'channel', heavy: true, top: { highlight: { tip: 0.7, back: 0.5 } } }),
  ...
  art({ id: 'tha', letters: 'ث', nameAr: 'الثاء', group: 'tip_between', duration: 3000,
    keyframes: fricative({ shape: 'tip_between', contact: L.teethEdge, contactKind: 'touch', airflow: 'channel', jaw: 0.3, top: { highlight: { tip: 1 }, forward: 1 } }),
  ...
  art({ id: 'zha', letters: 'ظ', nameAr: 'الظاء', group: 'tip_between', duration: 3000,
    keyframes: fricative({ shape: 'tip_between_heavy', contact: L.teethEdge, contactKind: 'touch', airflow: 'channel', jaw: 0.3, heavy: true, top: { highlight: { tip: 1, back: 0.5 }, forward: 1 } }),
    ruleId: 's3r8', refImages: ['tha_dhal_zha', 'heavy'] }),

  // ---- الشفتان ------------------------------------------------------------------
  art({ id: 'fa', letters: 'ف', nameAr: 'الفاء', group: 'lips', duration: 3000,
    keyframes: fricative({ shape: 'rest', lips: 'teeth_on_lip', jaw: 0.25, airflow: 'lips_channel' }),
    ruleId: 's3r9', refImages: ['lips_fa'] }),
  art({ id: 'ba', letters: 'ب', nameAr: 'الباء', group: 'lips',
    keyframes: plosive({ shape: 'rest', lips: 'closed', jaw: 0.1 }),
    ruleId: 's3r9', refImages: ['lips_closed'] }),
  art({ id: 'mim', letters: 'م', nameAr: 'الميم', group: 'lips',
    keyframes: nasal({ shape: 'rest', lips: 'closed', jaw: 0.1 }),
    ruleId: 's3r9', refImages: ['lips_closed', 'ghunnah'] }),
  art({ id: 'waw', letters: 'و', nameAr: 'الواو', group: 'lips',
    keyframes: fricative({ shape: 'waw', lips: 'rounded', jaw: 0.3, airflow: 'oral' }),
  ...
  art({ id: 'imalah', letters: 'النَّارِ', nameAr: 'ألف ممالة (إمالة)', group: 'vowel', duration: 3200, harakat: 2, tickerLabel: 'مد',
    keyframes: vowel({ shape: 'imalah', lips: 'narrow', jaw: 0.45, airflow: 'jawf', contact: L.imalahRing, contactKind: 'near', top: { highlight: { middle: 0.6 } } }),
  ...
  art({ id: 'heavy', letters: 'خُصَّ ضَغْطٍ قِظْ', nameAr: 'حرف غليظ (مفخم)', group: 'vowel', duration: 3000,
    keyframes: vowel({ shape: 'heavy', lips: 'open', jaw: 0.6, heavy: true, top: TOP_HEAVY }),
    ruleId: 's5r1', refImages: ['heavy'] }),
  art({ id: 'light', letters: 'رقيق', nameAr: 'حرف رقيق (مرقق)', group: 'vowel', duration: 3000,
    keyframes: vowel({ shape: 'light', lips: 'open', jaw: 0.6 }),
    ruleId: 's5r1', refImages: ['light'] }),
  art({ id: 'lam_allah_heavy', letters: 'هُوَ اللهُ', nameAr: 'لام (الله) غليظة', group: 'vowel', duration: 3000,
    keyframes: fricative({ shape: 'tip_ridge_heavy', contact: L.ridge, contactKind: 'touch', airflow: 'lateral', heavy: true, top: { highlight: { front_edge: 1, back: 0.5 }, lateral: 1 } }),
